...
Public Class ItemComponants
    ...
    Sub LoadGroups()
        Try
            ...
            Dim dt As DataTable = bm.ExcuteAdapter("LoadGroups2", New String() {"Form"}, New String() {0})
    ...
    Private Sub LoadTypes(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            Dim dt As DataTable = bm.ExcuteAdapter("LoadTypes2", New String() {"GroupId", "Form"}, New String() {xx.Tag.ToString, 0})
            For i As Integer = 0 To dt.Rows.Count - 1
                Dim x As New Button
                SetStyle(x)
                'bm.SetImage(x, CType(dt.Rows(i)("Image"), Byte()))
    ...
    Sub LoadAllItems()
        Try
            HelpDt = bm.ExcuteAdapter("Select cast(Id as nvarchar(100))Id,Name,SalesPrice Price From Items  where IsStopped=0 " & ItemWhere())
    ...
    Private Sub LoadItems(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            Dim dt As DataTable = bm.ExcuteAdapter("Select * From Items  where IsStopped=0 " & ItemWhere() & " and GroupId=" & WTypes.Tag.ToString & " and TypeId=" & xx.Tag.ToString)
            For i As Integer = 0 To dt.Rows.Count - 1
                Dim x As New Button
                SetStyle(x)
                'bm.SetImage(x, CType(dt.Rows(i)("Image"), Byte()))
...
Br:
            ...
            Dim dt As DataTable = bm.ExcuteAdapter("Select * From Items  where IsStopped=0 and Id='" & Id & "' " & ItemWhere())
    ...
    Sub FillControls()
        ...
        Dim dt As DataTable = bm.ExcuteAdapter("select * from " & TableName & " where " & SubId & "=" & ItemId.Text)
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If bm.ShowDeleteMSG() Then
            ...
            bm.ExcuteNonQuery("delete from " & TableName & " where " & SubId & "='" & ItemId.Text.Trim & "'")
