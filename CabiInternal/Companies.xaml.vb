...
Public Class Companies
    ...
    Sub DeleteDetails()
        bm.ExcuteNonQuery("delete from " & TableDetailsName & " where CompanyId='" & txtID.Text.Trim & "'      delete from " & TableDetailsName2 & " where CompanyId='" & txtID.Text.Trim & "'")
    ...
    Private Sub FillGrid()
        dt = bm.ExcuteAdapter("GetDoctorsCompanies", {"CompanyId"}, {Val(txtID.Text)})
        G.Rows.Clear()
    ...
    Private Sub FillGrid2()
        dt = bm.ExcuteAdapter("GetServiceCompanies", {"CompanyId"}, {Val(txtID.Text)})
        'dt = bm.ExcuteAdapter("select C.ServiceGroupId,dbo.GetServiceGroupName(C.ServiceGroupId)ServiceGroupName,C.Id,dbo.GetServiceTypeName(C.ServiceGroupId,C.Id)ServiceTypeName,isnull(T.Price,C.DrValue+C.CsValue+C.CoValue)Price,isnull(T.Payed,C.DrValue+C.CsValue+C.CoValue)Payed,isnull(T.Remaining,0)Remaining,isnull(T.Notes,'')Notes from ServiceTypes C left join ServiceCompanies T on(T.CompanyId='" & txtID.Text.Trim & "' and T.ServiceGroupId=C.ServiceGroupId and T.Id=C.Id)")
        G2.Rows.Clear()
        For i As Integer = 0 To dt.Rows.Count - 1
    ...
    Private Sub btnUpdate_Click(sender As Object, e As RoutedEventArgs) Handles btnUpdate.Click
        If bm.ShowDeleteMSG("سيتم تحديث البيانات .. استمرار؟") Then
            ...
            '    G.Rows(i).Cells(GC.DetectionPrice).Value *= (100 - Val(Perc.Text)) / 100
            '    G.Rows(i).Cells(GC.ConsultationPrice).Value *= (100 - Val(Perc.Text)) / 100
            'Next
