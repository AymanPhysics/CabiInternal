...
Public Class BankCash_G3
    ...
    Sub FillControls()
        ...
        Dim dt As DataTable = bm.ExcuteAdapter("select * from " & TableName & " where " & SubId & "=" & txtFlag.Text.Trim & " and " & SubId2 & "=" & txtID.Text)
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        Try
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.FromLinkFile OrElse G.Columns(e.ColumnIndex).Name = GC.FromSubAccNo Then
                dt = bm.ExcuteAdapter("select * from LinkFile where Id=" & Val(G.Rows(e.RowIndex).Cells(GC.FromLinkFile).Value))
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.ToLinkFile OrElse G.Columns(e.ColumnIndex).Name = GC.ToSubAccNo Then
                dt = bm.ExcuteAdapter("select * from LinkFile where Id=" & Val(G.Rows(e.RowIndex).Cells(GC.ToLinkFile).Value))
    ...
    Private Sub GridKeyDown(ByVal sender As Object, ByVal e As Forms.KeyEventArgs)
        ...
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.FromSubAccNo).Index Then
                dt = bm.ExcuteAdapter("select * from LinkFile where Id=" & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.FromLinkFile).Value))
            ...
            ElseIf G.CurrentCell.ColumnIndex = G.Columns(GC.ToSubAccNo).Index Then
                dt = bm.ExcuteAdapter("select * from LinkFile where Id=" & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.ToLinkFile).Value))
